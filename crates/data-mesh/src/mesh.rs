    ...
    if url.to_ascii_lowercase().contains(BANNED_CONSUMER_DOMAIN) {
        return Err(Error::Config(format!(
            "data-mesh consumers must not bind to lornu.ai URLs ({url}). \
             Use {IN_CLUSTER_URL} (in-cluster) or set DATA_FABRIC_URL to a reachable data-mesh."
        )));
    }
    Ok(())
}
...
impl ClientConfig {
    ...
    ///
    /// - `DATA_FABRIC_URL` (or legacy `DATAMESH_URL`) — base URL; defaults to the
    ///   in-cluster service.
...
impl Client {
    pub fn new(config: ClientConfig) -> Self {
        Self {
            config,
            // A data-mesh outage must not leave every consumer request
            // waiting forever. Keep these bounds below the AIVCS forge's
            // metadata timeout so handlers can return a useful backend error.
            http: HttpClient::builder()
                .connect_timeout(Duration::from_secs(10))
                .timeout(Duration::from_secs(30))
                .build()
                .expect("static data-mesh HTTP client configuration is valid"),
        }
    }
