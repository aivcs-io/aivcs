...
/// Parse `aivcs://org/repo`, `https://<host>/org/repo`, `org/repo`, or `org/repo@branch` into slug + branch.
pub fn parse_clone_target(url_or_slug: &str, branch_override: &str) -> Result<(String, String)> {
    let mut slug = url_or_slug.trim();
    if let Some(rest) = slug.strip_prefix("aivcs://") {
        slug = rest;
    } else if let Some(rest) = slug.strip_prefix("https://").or_else(|| slug.strip_prefix("http://")) {
        // Strip host and port to extract path (e.g., host.example.com/org/repo -> org/repo)
