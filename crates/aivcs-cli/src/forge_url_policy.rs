...
mod tests {
    ...
    fn https_any_host_is_allowed() {
        assert!(is_allowed_forge_url("https://aivcsd.aivcs.io", false));
        assert!(is_allowed_forge_url(
            "https://aivcs-forge-pg.tailbab6b0.ts.net",
            false
        ));
    ...
    fn plain_http_public_host_is_rejected() {
        assert!(!is_allowed_forge_url("http://aivcsd.example.test", false));
        assert!(!is_allowed_forge_url("http://aivcsd.example.test", true));
    }
    ...
    fn tailscale_magicdns_https_allowed_without_flag() {
        assert!(is_allowed_forge_url(
            "https://aivcs-core.tailbab6b0.ts.net",
            false
        ));
    }
    ...
    fn forge_service_url_tls_uses_https_scheme() {
        assert_eq!(
            forge_service_url("aivcs-forge-pg", "aivcs-forge-pg", 443, true),
