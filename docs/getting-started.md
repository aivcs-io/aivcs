...
On a laptop with a display, `aivcs login` uses **PKCE** against `issuer.aivcs.io`
(set `AIVCS_ISSUER_URL` / `--issuer` as needed). Headless environments use
`aivcs login --device`: open the verification URL, sign in, then **type** the
user code from the terminal (query-param prefill is not accepted as approval).
...
aivcs pr pipeline --branch feature/x --path docs/x.md --file ./x.md \
  --message "docs: x" --title "feat: x" --body "…" --owner aivcs-io --repo aivcs
```
